...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $email = $row['email'];
    $password = $row['password'];  // This will be used only if password change is not requested
} else {
    die("User not found.");
}
...
if (isset($_POST['submit'])) {
    ...
    // Hash the password if it's changed, otherwise keep the old one
    $password = $_POST['password'];
    if (!empty($password)) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT); // Hashing the password
    } else {
        $hashed_password = $password; // No change in password
    }
    ...
    $sql_update = "UPDATE admin SET 
                   ...
                   WHERE username='$_SESSION[login_admin]'";

    if (mysqli_query($db, $sql_update)) {
        $_SESSION['login_admin'] = $username; // Update session username if changed
        echo "<script>
...
<body>
    ...
    <div class="form-container">
        <form method="post">
            ...
            <input type="password" class="form-control" name="password" placeholder="Leave blank to keep current password"><br>
